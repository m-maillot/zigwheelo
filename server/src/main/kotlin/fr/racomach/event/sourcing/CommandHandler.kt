...
class CommandHandler(
    ...
    fun apply(command: Command): Either<Exception, AggregateId> =
        loadEvents(command)
            .flatMap { callAggregatorAndDispatchEvents(command, it) }
            .map { command.id }

    private fun callAggregatorAndDispatchEvents(command: Command, history: EventHistory): Either<Exception, Unit> =
    ...
    private fun loadEvents(command: Command): Either<Exception, EventHistory> =
